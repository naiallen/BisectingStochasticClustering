function [index, data1, data2] = EM_wishart( imacov_vec, L, q )
s = size(imacov_vec, 1);

% [ seed, ~, ~ ] = get_seeds_pca( imacov_vec );
[ seed, ~, ~ ] = get_seeds_random( imacov_vec );
...
while (error > epsilon)
    ...
    for ii = 1:step:size(imacov_vec,1)
        ...
        p_cluster1 = log(p{1}.weight) - getLogWhishart( cov, p{1} );
        p_cluster2 = log(p{2}.weight) - getLogWhishart( cov, p{2} );
